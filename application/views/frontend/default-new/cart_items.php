...
<?php foreach($cart_items as $cart_item): ?>
	...
	<?php 
		if($cart_item['type']=="course"){
			$course_details = $this->crud_model->get_course_by_id($cart_item['id'])->row_array();

		}elseif($cart_item['type']=="section"){
			$course_details = $this->crud_model->get_section_by_id($cart_item['id'])->row_array();

		}else{
			$course_details = $this->crud_model->get_lession_by_id($cart_item['id'])->row_array();
	      ...
	      <a href="<?php echo site_url('home/course/'.slugify($course_details['title']).'/'.$course_details['id']); ?>" class="checkPropagation">
	        <div class="menu_pro_wish-flex">
	          <div class="img checkPropagation">
	            ...
	            <span onclick="actionTo('<?php echo site_url('home/handle_cart_items/'.$course_details['id'].'/' . 'course') ?>');" class="cart-minus checkPropagation rounded-circle m-0 p-0 w-auto h-auto"><i class="fa-solid fa-minus m-1 text-9px"></i></span>
	          </div>
	          <div class="text">
