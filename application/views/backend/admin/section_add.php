...
<script type="text/javascript">
    function isFreeCourseChecked1(elem) {
    ...
    function calculateDiscountPercentage1(discounted_price) {
        if (discounted_price > 0) {
            var actualPrice = jQuery('#price1').val();
            if (actualPrice > 0) {
                var reducedPrice = actualPrice - discounted_price;
                var discountedPercentage = (reducedPrice / actualPrice) * 100;
                if (discountedPercentage > 0) {
                    jQuery('#discounted_percentage1').text(discountedPercentage.toFixed(2) + '%');

                } else {
                    jQuery('#discounted_percentage1').text('<?php echo '0%'; ?>');
                }
            }
        }
    }
