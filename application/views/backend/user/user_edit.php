    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body">
                ...
                <form class="required-form" action="<?php echo site_url('user/users/edit/'.$user_id); ?>" enctype="multipart/form-data" method="post">
                            ...
                            <div class="tab-pane" id="basic_info">
                                <div class="row">
                                    <div class="col-12">
                                        ...
                                        <?php $categories = $this->db->where('parent', '0')->get('category')->result_array();
                                        ?>
                                        <div class="form-group row mb-3">
                                            <div class="col-3">
...
<script>
    ...
    function loadSections() {
        ...
        $.ajax({
            url: '<?php echo site_url('user/ajax_get_sub_category'); ?>', // Adjust this to your route
            type: 'POST',
            dataType: 'json',
            data: {
                course_id: course_id // Adjusted to pass category_id
            },
            success: function(response) {
                $('#multiple_course_id2').empty();
                var subcatValue = <?php echo json_encode($user_data['subcat']); ?>; // Get PHP variable value in JavaScript
    
                $.each(response, function(key, value) {
                    var selected = (subcatValue == value.id) ? 'selected' : ''; // Check if the option should be selected
                    $('#multiple_course_id2').append('<option value="' + value.id + '" ' + selected + '>' + value.title + '</option>');
                });
            }
        });
    }
