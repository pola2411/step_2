...
if ( ! function_exists('get_active_course_purchases'))
{

function get_active_course_purchases($course_id = "", $user_id = "") {
    $CI =& get_instance();
    $CI->load->library('session');
    $CI->load->database();
    ...
    // Fetch active course purchases
    $course_purchases = $CI->db->select('*')
                               ->from('enrol')
                               ->where([
                                   'user_id' => $user_id,
                                   'course_id' => $course_id
    ...
    $active_purchases = [];
    foreach ($course_purchases->result() as $purchase) {
        $expiry_date = $purchase->expiry_date;
        if ($expiry_date == null || strtotime($expiry_date) >= time()) {
            $active_purchases[] = $purchase;  // Collect active purchases
        }
    }

    return $active_purchases;  // Return the list of active purchases
}
}
// ------------------------------------------------------------------------
