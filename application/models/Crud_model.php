...
class Crud_model extends CI_Model
{
    ...
    public function enrol_history_by_date_range_user($timestamp_start = "", $timestamp_end = "")
    {
        $this->db->select('enrol.*'); // Selecting all columns from the 'enrol' table
        $this->db->from('enrol');
        $this->db->join('users', 'users.id = enrol.user_id'); // Joining 'users' table with 'enrol' table
        $this->db->where('enrol.date_added >=', $timestamp_start);
        $this->db->where('enrol.date_added <=', $timestamp_end);
        $this->db->where('users.instractor_id', $this->session->userdata('user_id')); // Corrected condition for instructor_id
    
        // Order by date_added in descending order
    ...
    public function get_section_by_id($course_id = "")
    {
        return $this->db->get_where('section', array('id' => $course_id));
    }

    public function get_lession_by_id($course_id = "")
    {

        return $this->db->get_where('lesson', array('id' => $course_id));
    }
    ...
    public function add_section($course_id)
    {
        ...
        $data['title'] = html_escape($this->input->post('title'));
        $data['price'] = $this->input->post('price1');
        $data['discount_flag'] = $this->input->post('discount_flag1');
        $data['discounted_price'] = $this->input->post('discounted_price1');
        $data['is_free_course'] = $this->input->post('is_free_course1');
    ...
    //         if ($purchased_course->type == 'course') {
    //             $course_id = $purchased_course->id;
    //             $course = $purchased_course->id;
    //         } elseif ($purchased_course->type == 'lesson') {
    //             $lesson =  $this->get_lession_by_id($purchased_course->id)->row_array();
    //             $lesson_ids[] = $lesson['id'];
    //             $course_id = $lesson['course_id'];
    //         } elseif ($purchased_course->type == 'section') {
    //             $section =  $this->get_section_by_id($purchased_course->id)->row_array();
    ...
    //         if ($course != null) {
    //             if ($this->db->get_where('enrol', ['user_id' => $enrol_user_id, 'course_id' => $course])->num_rows() == 0) {
    //                 if ($payer_user_id) {
    ...
    //                 if ($this->db->get_where('enrol', ['user_id' => $enrol_user_id, 'sections' => $value])->num_rows() == 0) {
    //                     if ($payer_user_id) {
    //                         $data['gifted_by'] = $payer_user_id;
    //                     } else {
    //                         $data['gifted_by'] = 0;
    ...
    //                 if ($this->db->get_where('enrol', ['user_id' => $enrol_user_id, 'lession' => $value])->num_rows() == 0) {
    //                     if ($payer_user_id) {
    //                         $data['gifted_by'] = $payer_user_id;
    //                     } else {
    //                         $data['gifted_by'] = 0;
    ...
    public function enrol_student($enrol_user_id, $payer_user_id = "")
    {
        ...
        foreach ($purchased_courses as $purchased_course) {
            ...
            if($purchased_course->type == 'course' || $purchased_course->type == 'lesson' || $purchased_course->type == 'section'){
            
                if($purchased_course->type == 'course'){
                    if ($this->db->get_where('enrol', ['user_id' => $enrol_user_id, 'course_id' => $purchased_course->id])->num_rows() == 0) {
                        if ($payer_user_id) {
                            $data['gifted_by'] = $payer_user_id;
                        } else {
                            $data['gifted_by'] = 0;
                        ...
                        $data['user_id'] = $enrol_user_id;
                        $data['course_id'] = $purchased_course->id;
                        $data['date_added'] = strtotime(date('D, d-M-Y'));
                        ...
                        $data['last_modified'] = time();
                        $this->db->where('course_id',  $purchased_course->id);
                        $this->db->where('user_id', $enrol_user_id);
                    ...
                    if ($this->db->get_where('enrol', ['user_id' => $enrol_user_id, 'lession' => $purchased_course->id])->num_rows() == 0) {
                        ...
                        if ($payer_user_id) {
                            ...
                            $data['user_id'] = $enrol_user_id;
                            $data['course_id'] = $course_id;
                            $data['lession'] = $purchased_course->id;
                            $data['date_added'] = strtotime(date('D, d-M-Y'));
                            $this->db->insert('enrol', $data);
                            unset($data);
                            ...
                            $data['last_modified'] = time();
                            $this->db->where('lession',  $purchased_course->id);
                            $this->db->where('user_id', $enrol_user_id);
                            $this->db->update('enrol', $data);
                            unset($data);
                        }
                ...
                elseif($purchased_course->type == 'section'){
                    if ($this->db->get_where('enrol', ['user_id' => $enrol_user_id, 'sections' => $purchased_course->id])->num_rows() == 0) {
                            if ($payer_user_id) {
                                $data['gifted_by'] = $payer_user_id;
                            } else {
                                $data['gifted_by'] = 0;
                            ...
                            $data['user_id'] = $enrol_user_id;
                            $data['course_id'] = $course_id;
                            $data['sections'] = $purchased_course->id;
                            $data['date_added'] = strtotime(date('D, d-M-Y'));
                            $this->db->insert('enrol', $data);
                            unset($data);
                        } else {
                            $data['last_modified'] = time();
                            $this->db->where('sections',  $purchased_course->id);
                            $this->db->where('user_id', $enrol_user_id);
                            $this->db->update('enrol', $data);
                            unset($data);
                        }
            ...
            if ($this->db->get_where('enrol', ['user_id' => $enrol_user_id, 'course_id' => $course_id])->num_rows() == 0) {
                if ($payer_user_id) {
                    $data['gifted_by'] = $payer_user_id;
                } else {
                    $data['gifted_by'] = 0;
    ...
    public function enrol_lession_student_manually()
    {
        $users_id   = $this->input->post('user_id');
        $course   = $this->input->post('course_id');
        $sections =$this->input->post('lession_id');
    ...
    public function course_purchase($user_id, $method, $amount_paid, $param1 = "", $param2 = "")
    {
        ...
        foreach ($purchased_courses as $purchased_course) {
            ...
            $data['course_id'] = $purchased_course->id;
            $course_details = $this->get_course_by_id($purchased_course->id)->row_array();
