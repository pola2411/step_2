...
private function fetch_item_details($id, $type)
{
  ...
  public function top_courses_get($top_course_id = "") {
      $response = array();
      if($_GET['auth_token'] != "null"){
        $auth_token = $_GET['auth_token'];
        $logged_in_user_details = json_decode($this->token_data_get($auth_token), true);
        if ($logged_in_user_details['user_id'] > 0) {
          $user = $this->db->get_where('users', array('id' => $logged_in_user_details['user_id']))->result_array();
          $level_id = $user[0]['subcat'];
          $top_courses = array();
          $top_courses = $this->api_model->top_courses_get($top_course_id,$level_id);
  ...
  public function my_assignment_get(){
    $course_id = $_GET['course_id'];
    if($course_id > 0){
        $this->db->where('course_id', $course_id);
    }
    $this->db->order_by('assignment_id', 'desc');
    $response = $this->db->get('assignment')->result_array();
    return $this->set_response($response, REST_Controller::HTTP_OK);
