...
class Home extends CI_Controller
{
    ...
    public function handle_cart_items($course_id = "", $type = "", $identifier = "")
    {
        if (!$this->session->userdata('cart_items')) {
            $this->session->set_userdata('cart_items', array());
        }
        ...
        $previous_cart_items = $this->session->userdata('cart_items');
        $cart_item = ['id' => $course_id, 'type' => $type]; // Include cart type
    
        $cart_item_exists = false;
    
        // Check if the course is already in the cart
        foreach ($previous_cart_items as $key => $item) {
            if ($item['id'] === $course_id && $item['type'] === $type) {
                unset($previous_cart_items[$key]); // Remove if it exists
                $cart_item_exists = true;
                break;
            }
        }
    
        // Add or remove the course from the cart based on existence
        if (!$cart_item_exists) {
            array_push($previous_cart_items, $cart_item);
        }
    ...
    public function handle_buy_now($course_id = "", $types = "")
    {
        if (!$this->session->userdata('cart_items')) {
            $this->session->set_userdata('cart_items', array());
        }
        ...
        $previous_cart_items = $this->session->userdata('cart_items');
        $cart_item = ['id' => $course_id, 'type' => $types]; // Include cart type
        
        $cart_item_exists = false;
    
        // Check if the course is already in the cart
        foreach ($previous_cart_items as $key => $item) {
            if ($item['id'] === $course_id && $item['type'] === $types) {
                unset($previous_cart_items[$key]); // Remove if it exists
                $cart_item_exists = true;
                break;
            }
    ...
    public function handleCartItemForBuyNowButton($course_id = "", $cart_type = "")
    {
        if (!$this->session->userdata('cart_items')) {
            $this->session->set_userdata('cart_items', array());
        }
        ...
        $previous_cart_items = $this->session->userdata('cart_items');
        $cart_item = ['id' => $course_id, 'cart_type' => $cart_type]; // Include cart type
    
        $cart_item_exists = false;
    
        // Check if the course is already in the cart
        foreach ($previous_cart_items as $key => $item) {
            if ($item['id'] === $course_id && $item['cart_type'] === $cart_type) {
                unset($previous_cart_items[$key]); // Remove if it exists
                $cart_item_exists = true;
                break;
            }
        ...
        if ($this->session->userdata('user_login')) {
            $this->payment_model->configure_course_payment();
            echo json_encode(['redirectTo' => site_url('home/shopping_cart')]);
        } else {
    ...
    function course_payment()
    {
        ...
        // Check if the gift option is selected
        if (isset($_POST['is_gift']) && $_POST['is_gift'] == 1) {
            $gift_email = $this->input->post('gift_email');
    
            // Validate the email
            if (filter_var($gift_email, FILTER_VALIDATE_EMAIL)) {
                $gift_to_user = $this->db->get_where('users', ['email' => $gift_email])->row();
                
                // Check if recipient is an existing user
                if ($gift_to_user) {
                    $this->session->set_userdata('gift_to_user_id', $gift_to_user->id);
                } else {
                    $this->session->unset_userdata('gift_to_user_id');
                    $this->session->set_flashdata('error_message', site_phrase('Recipient must be an existing user'));
                    redirect(site_url('home/shopping_cart'), 'refresh');
                }
            } else {
                $this->session->unset_userdata('gift_to_user_id');
