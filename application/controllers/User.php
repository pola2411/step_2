...
class User extends CI_Controller
{
    ...
    public function users($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
            $this->user_model->add_user(false, false, true);
    ...
    function server_side_users_data()
    {
        ...
        $total_number_of_row = $this->db->where('role_id !=', 1)->where('is_instructor', 0)->where('is_instructor',$this->session->userdata('user_id'))->get('users')->num_rows();
        ...
        if (empty($search)) {
            ...
            $this->db->where('instractor_id =', $this->session->userdata('user_id'));
        ...
        foreach ($students as $key => $student) :
            ...
            $action = '<div class="dropright dropright">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-item" href="' . site_url('user/user_form/edit_user_form/' . $student['id']) . '">' . get_phrase('edit') . '</a></li>

                                <li><a class="dropdown-item" href="#" onclick="confirm_modal(&#39;' . site_url('user/users/delete/' . $student['id']) . '&#39;);">' . get_phrase('delete') . '</a></li>
    ...
    public function enrol_history($param1 = "")
    {
        ...
        $page_data['enrol_history'] = $this->crud_model->enrol_history_by_date_range_user($page_data['timestamp_start'], $page_data['timestamp_end']);
    ...
    {
        $response = array();
        $result = $this->db->where('role_id !=', 1)->where('instractor_id',$this->session->userdata('user_id'))->group_start()->like('first_name', $_GET['searchVal'])->or_like('last_name', $_GET['searchVal'])->or_like('email', $_GET['searchVal'])->group_end()->limit(100)->get('users')->result_array();
        if ($default != '') {
